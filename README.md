...
| Third-party | **site-packages/dist-packages tự phát hiện** (`kamipy paths`): package thuần Python đủ đơn giản được **biên dịch thẳng**; còn lại (flask, numpy, PIL, ...) **bridge qua libpython nhúng**: import/call/method/kwargs, `@app.route` decorator nhận **compiled function làm Python callable**, `class Bot(ZaloAPI)` **subclass class Python từ code biên dịch** (type() + PyInstanceMethod), iterate/index/attr-set. `KAMIPY_SITE_PACKAGES`/`KAMIPY_LIBPYTHON` để override |
...
Not yet / Chưa hỗ trợ trong mã *biên dịch* (lỗi thông báo rõ): walrus `:=`, `@staticmethod/@property`, `nonlocal`, metaclasses — nhưng package third-party dùng những thứ đó (**flask, numpy, PIL, requests bản pip, zlapi, ...**) vẫn chạy được qua **CPython bridge**: compiler tự phát hiện package trong site-packages, thử biên dịch, không được thì nhúng libpython và bridge trong suốt (call/attr/kwargs/subclass/callback).

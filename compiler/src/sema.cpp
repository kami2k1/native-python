...
struct Sema {
    ...
    // A helpful diagnostic: what *is* importable, and what to do about a
    // third-party package (pyautogui, flask, ...) that kamipy has never seen.
    std::string unknown_module_message(const std::string& modname) const {
    ...
    void register_import(Stmt* s) {
        ...
        if (modname.find('.') != std::string::npos || !modules().count(modname)) {
            ...
            err(s->line, unknown_module_message(modname));
        }
